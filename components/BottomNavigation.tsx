...
const NavItem = ({
  ...
  return (
    ...
    >
      ...
      {route.path === '/carts' && (
        <View className="w-[20px] h-[20px] bg-red-500 absolute top-1 right-2 z-10 rounded-full justify-center items-center pt-0.5">
          <Text className={`text-white font-bold ${cartCount >= 10 ? 'text-xs' : 'text-sm'}`}>
            {cartCountFormatted()}
          </Text>
        </View>
      )}
